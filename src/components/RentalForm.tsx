    ...
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="name" className="block text-body font-semibold text-neutral-700 mb-2">Ім&apos;я*</label>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="social" className="block text-body font-semibold text-neutral-700 mb-2">Телеграм/Інстаграм</label>
            <input id="social" className="input px-3 py-2 text-body w-full" placeholder="@username або t.me/username" {...register("social")} />
          </div>
        </div>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="plan" className="block text-body font-semibold text-neutral-700 mb-2">План</label>
            <select id="plan" className="input px-3 py-2 text-body w-full" {...register("plan")}>
              <option>Mini</option><option>Maxi</option>
            </select>
          </div>
          <div>
            <label htmlFor="bookId" className="block text-body font-semibold text-neutral-700 mb-2">Книга (ID)</label>
        ...
        {/* Способ оплаты */}
        <div>
          <label className="block text-body font-semibold text-neutral-700 mb-2">Спосіб оплати *</label>
          <div className="grid md:grid-cols-2 gap-3">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className={`font-semibold text-body ${paymentMethod === 'card' ? 'text-blue-900' : 'text-neutral-700'}`}>
                    Переказ на карту Monobank
                  </p>
                  <p className={`text-caption ${paymentMethod === 'card' ? 'text-blue-700' : 'text-neutral-500'}`}>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className={`font-semibold text-body ${paymentMethod === 'online' ? 'text-green-900' : 'text-neutral-700'}`}>
                    Онлайн оплата
                  </p>
                  <p className={`text-caption ${paymentMethod === 'online' ? 'text-green-700' : 'text-neutral-500'}`}>
        ...
        {paymentMethod === 'card' && (
          <div>
            <label htmlFor="screenshot" className="block text-body font-semibold text-neutral-700 mb-2">Скріншот оплати *</label>
            <input
              {...register("screenshot", {
              ...
              type="file"
              accept="image/*"
              className="w-full input file:mr-4 file:py-2 file:px-4 file:rounded-2xl file:border-0 file:text-body file:font-semibold file:bg-neutral-100 file:text-neutral-700 hover:file:bg-neutral-200"
            />
        ...
        <div>
          <label htmlFor="note" className="block text-body font-semibold text-neutral-700 mb-2">Коментар</label>
        ...
        <div className="flex items-start gap-3">
          <input
            ...
            id="privacyConsent"
            type="checkbox"
            className="mt-1 w-4 h-4 text-brand-accent-light bg-neutral-100 border-neutral-300 rounded focus:ring-brand-accent focus:ring-2"
          />
        ...
        <button type="submit" className="btn-base bg-[--ink] text-neutral-0 rounded-xl h-10 px-6 text-body font-semibold mt-2">Надіслати заявку</button>
      </form>
    </div>
  );
}
