...
interface OptimizedDataPlatformProps<T> {
  data: T[]
...
const OptimizedDataPlatform = memo(function OptimizedDataPlatform<T>({
  ...
  useEffect(() => {
    ...
    if (!isSecure) {
      alerts.push('Небезпечні дані виявлено')
    }
    
    if (performanceMetrics.securityLevel < 80) {
      alerts.push('Рівень безпеки потребує покращення')
    }
    
    setSecurityAlerts(alerts)
  }, [isSecure, performanceMetrics.securityLevel])
  ...
  useEffect(() => {
    ...
    if (!isPrivate) {
      alerts.push('Приватні дані можуть бути скомпрометовані')
    }
    
    if (performanceMetrics.privacyLevel < 80) {
      alerts.push('Рівень приватності потребує покращення')
    }
    
    setPrivacyAlerts(alerts)
  }, [isPrivate, performanceMetrics.privacyLevel])
...
export default OptimizedDataPlatform
