...
export async function POST() {
  try {
    ...
    logger.info('Setting up categories data', {}, 'Database');
    ...
    const ageCategory = createdCategories?.find(c => c.slug === 'age');
    const genreCategory = createdCategories?.find(c => c.slug === 'genre');
    const adultCategory = createdCategories?.find(c => c.slug === 'adults');
    ...
    return NextResponse.json({
      success: true,
      message: 'Categories data setup completed successfully',
      results: {
        totalCategories: allCategories?.length || 0,
        parentCategories: createdCategories?.length || 0,
        subcategories: createdSubcategories?.length || 0,
    ...
    logger.error('Categories setup failed', error, 'Database');
    return NextResponse.json({ 
      success: false, 
      error: 'Categories setup failed',
