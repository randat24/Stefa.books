...
export async function POST() {
  try {
    ...
    logger.info('Creating test categories data', {}, 'Database');
    
    // Сначала попробуем создать простую таблицу через insert
    const testCategory = {
      id: '00000000-0000-0000-0000-000000000001',
      name: 'Тестова категорія',
      slug: 'test-category',
      description: 'Тестова категорія для перевірки',
      icon: '🧪',
      color: '#FF0000',
      parent_id: null,
      sort_order: 1,
    ...
    // Попробуем вставить тестовую категорию
    ...
    if (testError) {
      logger.error('Error creating test category', { error: testError }, 'Database');
      return NextResponse.json({ 
        success: false,
        error: 'Categories table does not exist',
        details: testError.message,
        suggestion: 'Please create the categories table manually in Supabase dashboard',
        sql: `
...
CREATE POLICY "Categories are insertable by service role" 
  ON public.categories FOR INSERT 
  WITH CHECK (true);
  
CREATE POLICY "Categories are updatable by service role" 
  ON public.categories FOR UPDATE 
  USING (true);
  
CREATE POLICY "Categories are deletable by service role" 
  ...
  USING (true);
    ...
    logger.info('Test category created successfully', {}, 'Database');
    
    // Если тестовая категория создалась, удаляем её и создаем реальные данные
    await supabase
      .from('categories')
      .delete()
    ...
    logger.info('Test categories creation completed successfully', { 
    ...
    logger.error('Test categories creation failed', error, 'Database');
    return NextResponse.json({ 
      success: false, 
      error: 'Test categories creation failed',
