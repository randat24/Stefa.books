...
// URL Google таблицы с Cloudinary ссылками
...
async function importCloudinaryCovers() {
  console.log('🚀 Starting Cloudinary covers import from Google Sheets...\n');
  ...
  try {
    ...
    if (sheetsData.length === 0) {
      console.log('❌ No Cloudinary data to process');
      return;
    }
    ...
    console.log('📋 Sample Cloudinary data from Google Sheets:');
...
async function testConnection() {
  ...
  if (sheetsData.length > 0) {
    ...
    console.log('❌ Google Sheets connection failed or no Cloudinary data found');
    return false;
  }
}

// Функция для показа текущих обложек в базе данных
...
async function main() {
  ...
  if (args.includes('--test')) {
    ...
    if (connectionOk) {
      console.log('\n' + '='.repeat(60));
      console.log('Proceeding with Cloudinary import...');
      console.log('='.repeat(60));
      await importCloudinaryCovers();
    } else {
      ...
      console.log('4. Make sure Cloudinary URLs are complete and valid');
      console.log('5. Example URL format: https://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/export?format=csv&gid=0');
    }
  }
}
