...
/**
 * Простой скрипт для вставки книг через fetch API
 * Без зависимостей от @supabase/supabase-js
 */
...
async function checkConnection() {
  try {
    const response = await fetch(`${supabaseUrl}/rest/v1/books?select=count&limit=1`, {
      headers: {
        'apikey': supabaseKey,
        'Authorization': `Bearer ${supabaseKey}`
      }
...
async function insertBooks() {
  ...
  for (let i = 0; i < booksData.length; i++) {
    ...
    try {
      console.log(`📖 Вставляем книгу ${i + 1}/${booksData.length}: "${book.title}" (${book.code})`);
      ...
      if (result.success) {
        ...
        console.error(`❌ Ошибка при вставке книги "${book.title}":`, result.error);
        errorCount++;
      }
...
async function main() {
  console.log('📚 Скрипт вставки книг в Stefa.Books (простая версия)\n');
