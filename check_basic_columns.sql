-- ============================================================================
-- ПРОВЕРКА БАЗОВЫХ КОЛОНОК ТАБЛИЦЫ BOOKS
-- ============================================================================
...
-- 2. Проверяем существование базовых колонок
SELECT 
    'Проверка базовых колонок:' as info,
    CASE WHEN EXISTS (
    ...
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'books' AND column_name = 'tags' AND table_schema = 'public'
    ) THEN 'tags: ЕСТЬ' ELSE 'tags: НЕТ' END as tags_column,
    
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'books' AND column_name = 'badges' AND table_schema = 'public'
    ) THEN 'badges: ЕСТЬ' ELSE 'badges: НЕТ' END as badges_column,
    
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'books' AND column_name = 'status' AND table_schema = 'public'
    ) THEN 'status: ЕСТЬ' ELSE 'status: НЕТ' END as status_column;
