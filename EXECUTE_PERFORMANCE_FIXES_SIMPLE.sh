...
# =====================================================
# Этот скрипт выполняет основные исправления производительности
...
if ! command -v psql &> /dev/null; then
    ...
    echo "На Ubuntu: sudo apt-get install postgresql-client"
    exit 1
fi
...
if pg_dump "$SUPABASE_URL" > "$BACKUP_FILE" 2>/dev/null; then
...
if psql "$SUPABASE_URL" -f "PERFORMANCE_FIXES_SIMPLE.sql"; then
...
- ✅ user_profiles.email → для быстрого поиска
...
WHERE schemaname = 'public'
    AND indexname LIKE 'idx_%';
" >> "$REPORT_FILE" 2>/dev/null || warning "Не удалось получить общую статистику"
...
echo "   • Улучшен полнотекстовый поиск"
echo ""
echo "📋 Следующие шаги:"
...
echo "   4. Рассмотрите удаление неиспользуемых индексов"
echo ""
